...
import {addItemToCart,gotoCart,removeItemFromWishlist,addItemToWishlist} from '../../dryProviders'
...
const ProductCard=({productObj})=>{
  ...
  const [inCart,setButtonText]=useState(false)
    ...
    const toggleWishlist=async(handlerObj)=>{
      try{      
        if(!wishListed){
        await addItemToWishlist(handlerObj,dispatchWishlist,setToWishlist,wishlistCount)
        }else{ 
        await removeItemFromWishlist(handlerObj,dispatchWishlist,setToWishlist,wishlistCount)  
        }
    }catch(e){
        throw e
      }
    ...
    const cartHandler=async(handlerObj)=>{
      try{
        ...
        switch(type){
          ...
          default:
            return await addItemToCart(payload,dispatchCart,setButtonText,cartCount)
        }
    ...
    <div className="flex-V-center-VH text-align-center" >
      ...
      <p>{description}</p>
      <h4>${price}</h4>
      <button  className={!inCart?"btn primary-btn p-card-btn":"btn primary-outlime-btn"} onClick={inCart?()=>cartHandler({type:"navigate_to_cart"}):()=>cartHandler({type:"add_to_cart",payload:productObj})} >{!inCart?'Add to cart':"Go to cart"}</button>        
    </div>
